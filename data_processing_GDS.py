...
class dataFrameASIC:
    ...
    def group_events(self,df_chunk):
        ...
        for x in range(1,12):
            for y in range(1,12):
                #i = 11*x+y
                df_sub_all = df_chunk[(df_chunk['x']==(x)) & (df_chunk['y']==(y))]
                df_sub = single_df_chunk[(single_df_chunk['x']== (x)) & (single_df_chunk['y']==(y))]
                subtotal, e = np.histogram(df_sub['Amplitude'].values, bins = self.mybins)
                #plt.figure()

                subtotal_all, sube = np.histogram(df_sub_all['Amplitude'].values, bins = self.mybins)
                #plt.plot(self.mybins[:-1],subtotal_all)
                #print(subtotal_all)
                #print(df_sub_all)
                #print(self.current_dataframe)
                #plt.figure()
                #self.channelSpectras[x][y] = self.channelSpectras[x][y].add(df_sub['Amplitude'].value_counts(), fill_value = 0 )
                self.channelSpectras[x][y] = self.channelSpectras[x][y]+subtotal
                self.channelSpectras_all[x][y] = self.channelSpectras_all[x][y]+subtotal_all
                #plt.plot(self.mybins[:-1],self.channelSpectras_all[x][y])
                #plt.show()
        ...
        with open(filename, "rb") as binary_file:
            ...
            while not_empty:
                ...
                #   values = np.array(values)
                    ...
                    #= np.append(values, 0)
                    if info[2] == 0:
                        values = self.subtract_pedestals_in_read_cathode(info[2],values,info[3],info[4], self.df_pedestals, self.df_gains, _cells, start_cell)
                    #if values[-1] == 0:
                    #    bad_events += 1
                    else:
    ...
    #    plt.xlabel('Cell')
        ...
        amplitude = calculate_amplitude_trapezoidal(input, 70, 14)*gain
...
#rel_path = '../logs/*'
rel_path = '../master-data/2020-12-18__Cs137_th1800_wNeigh__GDS-100__raw_data_log.bin'
